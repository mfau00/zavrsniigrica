...
class Game:
    ...
    def check_collision(self):
        ...
        if pogodak:
            # moramo generirat nove neprijatelje nakon sto pogodimo metu jer se iz grupe sprite.group uklanjaju baloni
        ...
        for enemy1 in self.enemy_sprites:
            if enemy1.rect.top > DISPLAY_HEIGHT - 10:
                self.started = False
                self.game_over = True
    ...
    def game_intro(self):
        ...
        while intro:
            ...
            self.message_to_screen("Welcome", LIGHTRED, -120, size="medium")
            self.message_to_screen("Press C to continue", LIGHTBLUE, -50, size="small")
            self.message_to_screen(" Q to quit", LIGHTBLUE, 0, size="small")
            pg.display.update()
...
class Player(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image = pg.image.load('cap.png')
    ...
    def move_player(self,keys):
        if keys[pg.K_LEFT]:
            self.image = pg.image.load('cap3-left.png')
            self.rect.move_ip(-self.move_x, 0)

        elif keys[pg.K_RIGHT]:
            self.image = pg.image.load('cap4-right.png')
            self.rect.move_ip(self.move_x, 0)
        else:
            self.image = pg.image.load('cap.png')
...
class Enemy(pg.sprite.Sprite):
    def __init__(self):
        ...
        self.image_names = ['enemy2.png', 'enemy4.png', 'thanos3.png', 'enemyfly.png']
    ...
    def different_scores(self, rand_img):
        if rand_img == 'thanos3.png':
            return 5
        elif rand_img == 'enemy4.png':
            return 2
        else:
            return 1
...
class Powerful_enemy(pg.sprite.Sprite):
    def __init__(self):
        pg.sprite.Sprite.__init__(self)
        self.image = pg.image.load('ufoskull.png')
        self.rect = self.image.get_rect()
        self.rect.x = random.randrange(0, DISPLAY_WIDTH - self.rect.width)
